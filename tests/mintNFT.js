...
console.log('VandalizeMe address:', colors.green(vandalizeMeContract.options.address))
console.log('CryptoVandals address:', colors.green(cryptoVandalsContract.options.address))
...
async function mint () {
    ...
    var rawTx = {
        ...
        "data": vandalizeMeContract.methods.mint(address, 'http://example.com/').encodeABI(),
    }
    var tx = new Tx(rawTx)
    tx.sign(privateKey)
    ...
    rawTx = {
        ...
        "gasPrice": web3.utils.toHex(web3.utils.toWei('20', 'gwei')),
        "gasLimit": web3.utils.toHex(2000000),
        "data": cryptoVandalsContract.methods.approve(cryptoVandalsContract.options.address, 0).encodeABI(),
    }
    tx = new Tx(rawTx)
    tx.sign(privateKey)
    try {
